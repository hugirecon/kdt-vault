...
export default function BulkActions({ count, onClear }: BulkActionsProps) {
  ...
  return (
    <div className="flex items-center gap-3 p-3 rounded-xl bg-blue-500/10 border border-blue-500/20 animate-slide-up">
      ...
      {[
        ...
        <button
          key={action.label}
          className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs text-white/50 hover:text-white/80 hover:bg-white/[0.05] transition-all duration-200"
        >
          {action.icon}
          {action.label}
        </button>
